...
from fedbr.test import helpers
from fedbr.scripts import collect_results
...
class TestCollectResults(unittest.TestCase):
    ...
    def test_end_to_end(self):
        ...
        python -m fedbr.scripts.collect_results --input_dir=fedbr/misc/test_sweep_data \
            | tee fedbr/misc/test_sweep_results.txt
        ...
        python -m fedbr.scripts.sweep launch --data_dir=$DATA_DIR \
          --output_dir=fedbr/misc/test_sweep_data --algorithms ERM \
          --datasets VLCS --steps 1001  --n_hparams 2 --n_trials 2 \
          --command_launcher local
        """
        result = subprocess.run('python -m fedbr.scripts.collect_results'
            ' --input_dir=fedbr/misc/test_sweep_data', shell=True,
            stdout=subprocess.PIPE)

        with open('fedbr/misc/test_sweep_results.txt', 'r') as f:
            ground_truth = f.read()
